...
class polygon_math:
    ...
    def calculate_polygon_mean_position(vertex_list):
        ...
        return center_coordinate.tolist()
...
class vertex_math:
    ...
    def calculate_rotation_angles(mouse_position, vertex_center_position):
        # Calculate direction vector from cube center to mouse position
        direction_vector = np.array(mouse_position) - np.array(vertex_center_position)
        
        # Normalize the direction vector
        direction_vector /= np.linalg.norm(direction_vector)
        
        # Calculate the angle in the x-z plane
        angle_xz = np.arctan2(direction_vector[0], direction_vector[2])
        
        # Calculate the angle in the y-z plane
        angle_yz = np.arctan2(direction_vector[1], direction_vector[2])
        
        # Convert angles to degrees and adjust the range to be between -180 and 180
        angle_xz_deg = np.degrees(angle_xz)
        angle_yz_deg = np.degrees(angle_yz)
        
        if angle_xz_deg > 180:
            angle_xz_deg -= 360
        elif angle_xz_deg < -180:
            angle_xz_deg += 360
        
        if angle_yz_deg > 180:
            angle_yz_deg -= 360
        elif angle_yz_deg < -180:
            angle_yz_deg += 360
        
        return angle_xz_deg, angle_yz_deg

class weak_perspective_projection:

    def translate_vertex_center_position(x:int, y:int, vertex_center: list[int,int,list]) -> list:
        x0, y0, z0 = vertex_center
        x1 = x0+x
        y1 = y0+y
        return [x1,y1,z0]
